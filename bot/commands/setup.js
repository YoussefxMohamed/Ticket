...
module.exports = {
    ...
    run: async function(client, message, args) {
        try {
            ...
            var title = message.content.split(' ').slice(3).join(' ') || 'Turbo Bot';
            if (!adminRole) {
                message.react('⚠');
                message.channel.send({
                    embed: {
                        title: `<a:glt:929681265752412180> | Wrong use`,
                        description: `<a:t3gb:929797409444343878> | correct use: ${prefix}setup <Ticket Channel> <Admins Role> <Ticket Message Title>`,
                        color: 0xFF0000
                    }
